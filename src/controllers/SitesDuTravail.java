...
public class SitesDuTravail extends Controller {
    ...
    public void enregistrer() {
        try {
            ...
            easy_sales.Pst = (PreparedStatement) easy_sales.cn.clientPrepareStatement("INSERT INTO Sites (idSite,libSite,adresse) "
                    + " (?,?,?)");
    ...
    public void modifier() {
        try {
            easy_sales.connexionEasy();
            easy_sales.Pst = (PreparedStatement) easy_sales.cn.clientPrepareStatement("UPDATE Sites SET libSite=?,adresse=? "
                    + " WHERE idSite = ?");
    ...
    public void supprimer() {
        try {
            easy_sales.connexionEasy();
            easy_sales.Pst = (PreparedStatement) easy_sales.cn.clientPrepareStatement("DELETE FROM Sites WHERE idSite = ?");
            easy_sales.Pst.setString(1, idSite);
            easy_sales.Pst.execute();
            easy_sales.deconnexionEasy();
    ...
    public boolean rechercherSite(){
        ...
        try {
            easy_sales.connexionEasy();
            easy_sales.Pst = (PreparedStatement) easy_sales.cn.clientPrepareStatement("SELECT idSite FROM Sites WHERE idSite = ?");
            easy_sales.Pst.setString(1, idSite);
